...
Deno.serve(async (req) => {
  try {
    ...
    // Formats: "online|v:1.0.0|b:Mar  1 2026 14:30:00", "offline", "ota_updating", "ota_success", "ota_failed"
    if (eventType === 'status') {
      const statusBytes = decodeBase64(payloadB64);
      const rawStatus = new TextDecoder().decode(statusBytes);

      // Parse "online|v:1.0.0|b:Mar  1 2026 14:30:00"
      ...
      for (let i = 1; i < parts.length; i++) {
        if (parts[i].startsWith('v:')) {
          ...
          // C __DATE__ __TIME__ format: "Mar  1 2026 14:30:00"
